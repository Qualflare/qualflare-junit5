...
## Turning the metadata API on

`Qualflare.label` and friends need the running test's context, which JUnit does not expose
ambiently. Either annotate:

```java
@ExtendWith(QualflareExtension.class)
```

or enable auto-detection once, in `junit-platform.properties`:

```properties
junit.jupiter.extensions.autodetection.enabled=true
```

Auto-detection is **off by default in JUnit**, which is why the annotation is the
documented path. Results are reported either way — only the metadata needs the extension.
